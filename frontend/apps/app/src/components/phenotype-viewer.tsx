          ...
          <ScrollArea className="h-[700px] w-full">
            {phenotypes.map((phenotype, index) => (
              <div key={phenotype.id} className="mb-4 p-4 border rounded-lg">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Created: {new Date(phenotype.created_at).toLocaleString()}
                </p>
              </div>
            ))}
          </ScrollArea>
        ...
        <CardContent className="flex items-center space-x-4">
          <Avatar className="h-24 w-24">
            <AvatarImage
              ...
              alt={`${patientData.patient.first_name} ${patientData.patient.last_name}`}
            />
            <AvatarFallback>
              {patientData.patient.first_name[0]}
              {patientData.patient.last_name[0]}
            </AvatarFallback>
          </Avatar>
          <div>
